...
| `capture.<PSR>.price` / `capture.<PSR>.factor` | Monthly capture price (generation-weighted achieved day-ahead price) and value factor (÷ the month's baseload mean) per technology — `capture.B16.factor` is the solar-cannibalisation series. One point per month at the 1st 00:00 UTC; complete months only, whole-month baseload denominator, sample guards per `backend/power/capture.py`. **Coverage:** volumes are ENTSO-E's aggregated actual generation — the TSO-visible fleet, not the national one (behind-the-meter PV absent; DE gas ≈ half of AGEB national statistics, Unnewehr et al. 2021); the API response carries a `coverage_note` | EUR/MWh / ratio |
