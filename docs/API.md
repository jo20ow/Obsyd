...
| `capture.<PSR>.price` / `capture.<PSR>.factor` | Monthly capture price (generation-weighted achieved day-ahead price) and value factor (÷ the month's baseload mean) per technology — `capture.B16.factor` is the solar-cannibalisation series. One point per month at the 1st 00:00 UTC; complete months only, whole-month baseload denominator, sample guards per `backend/power/capture.py` | EUR/MWh / ratio |
