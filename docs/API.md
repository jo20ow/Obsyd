...
| `price.ida<1\|2\|3>.qh` | Intraday-auction clearing prices (SIDC IDA1-3, 15-min products; IDA3 covers the afternoon only — the market's span, not a gap). TP submission is voluntary: currently **Spain only, from 2026-01** — the coverage probe lives in `docs/findings/2026-09-13-ida-intraday-coverage.md`. Continuous intraday remains licensed and absent | EUR/MWh |
| `spread.da_imbalance` | Imbalance settlement price − day-ahead auction price, per hour (the European DART reading: what being out of balance cost vs. having bought day-ahead; positive = short was dearer). Only hours where both legs exist; structurally absent for GB (no auction feed) | EUR/MWh |
| `price.negative_hours` | Daily count of negative day-ahead hours (resolution-weighted: four negative quarter-hours = one hour), one point per day at 00:00 UTC — the same number the desk's day-ahead panel marks, promoted to an exportable series. Absent for zones without a day-ahead feed (GB) | h |
| `spread.tb<1\|2\|4>` | Daily day-ahead **top-bottom spread**: the sum of the day's n highest hourly prices minus the sum of its n lowest (n = 1/2/4, mapping to 1h/2h/4h storage at one cycle per UTC day; days under 20 priced hours skipped). **A price-spread statistic, NOT a battery revenue benchmark** — real assets stack intraday, balancing and ancillary revenue (GB 2h systems earned ~1.4× TB2 in 2025), and naive day-ahead trading does not capture the full perfect-foresight spread either. No efficiency applied (pure arithmetic, reproducible); UTC-day buckets uniformly across zones. Absent for GB (no auction series — a TB on the MID index would be a different statistic) | EUR/MW-day |
| `capture.<PSR>.price` / `capture.<PSR>.price_floor0` / `capture.<PSR>.factor` | Monthly capture price (generation-weighted achieved day-ahead price), its **€0-floor variant** (negative hours priced at zero — the spread between the two is the technology's negative-price exposure in EUR/MWh; two named variants instead of one contested number, and neither is a "PPA value"), and the value factor (÷ the month's baseload mean) per technology — `capture.B16.factor` is the solar-cannibalisation series. One point per month at the 1st 00:00 UTC; complete months only, whole-month baseload denominator, sample guards per `backend/power/capture.py`. **Coverage:** volumes are ENTSO-E's aggregated actual generation — the TSO-visible fleet, not the national one (behind-the-meter PV absent; DE gas ≈ half of AGEB national statistics, Unnewehr et al. 2021); the API response carries a `coverage_note` | EUR/MWh / ratio |
| `congestion.cost.security` / `congestion.cost.countertrading` | German congestion-management costs (SMARD/Bundesnetzagentur, CC BY 4.0, attribute "Bundesnetzagentur \| SMARD.de"): the TSOs' monthly network-security bundle (redispatch incl. feed-in management, reserves) and countertrading, in EUR, under DE_LU, since 2022-07. Published with a 3-4 month lag; SMARD's own split, deliberately not re-labelled as redispatch-only | EUR |
| `conv.<full\|low\|hours>.<ZONE>` / `conv.spread.<ZONE>` | Daily day-ahead price-convergence statistics per border, stored under the border's sorted-first zone (the `flow.<TO>` convention), one point per UTC day. Bands are **ACER's MMR 2024 definitions**: `full` = hours with \|spread\| ≤ 1 EUR/MWh, `low` = hours > 10 EUR/MWh, `hours` = hours priced on both sides (moderate derives exactly as `hours − full − low`); `spread` = the day's mean absolute spread. Counts, not shares, so any window aggregates exactly (DST days have 23/25 h). ACER's own caveat travels with the data: full convergence is not an objective. Aggregated view: `/api/power/convergence` | h / EUR/MWh |
| `co2.intensity.lifecycle` / `co2.intensity.direct` | **Estimated** CO₂ intensity of the zone's own generation — the published `gen.<PSR>` mix times per-technology emission factors (life-cycle: IPCC AR5 Annex III medians via Electricity Maps' AGPL factor set; direct: the operational set). Production-based (imports ignored), technology-average (±10–20% vs plant-calibrated factors), storage discharge excluded ≡ priced at the hour's own average. Full methodology, factor table with per-value sources, and every classless-PSR mapping: `backend/power/co2.py` | gCO2eq/kWh |
