...
def fetch_port_data(port_id: str, days: int = 30) -> list[dict]:
    ...
    if not re.match(r"^[a-zA-Z0-9_]+$", port_id):
        raise ValueError(f"Invalid port_id: {port_id}")
    params = {
        "where": f"portid = '{port_id}'",
        "outFields": "portid,portname,date,portcalls,portcalls_tanker,import_tanker,export_tanker,import,export",
        "orderByFields": "date DESC",
...
def fetch_disruptions(days: int = 90) -> list[dict]:
    ...
    params = {
        ...
        "outFields": "eventid,eventname,eventtype,alertlevel,fromdate,todate,affectedports,country,htmldescription",
...
if __name__ == "__main__":
    ...
    if cmd == "backfill":
        ...
        print("  ArcGIS paginates at 2000 records — this may take a few minutes.\n")
        ...
        for a in avgs:
            print(
                f"  {a['portname']:<25} {a['n_days']:>5} {a['avg_total']:>10.1f} {a['avg_tanker']:>11.1f} {a['avg_capacity']:>14,.0f}"
            )
        ...
        if active:
            ...
            for d in active[:10]:
                print(
                    f"  {d['alertlevel']:>5} {d['event_type']:<12} {d['event_name'][:38]:<40} {(d['affected_portname'] or '')[:18]:<20}"
                )
        else:
