...
// value is shown verbatim in the tooltip — never dressed up as "forced".
const rank = (e) => (e.kind === 'forced' ? (e.running_now ? 3 : 2) : 1)

const EMPTY = { paths: [], counts: { total: 0, drawable: 0, undrawable: 0, running: 0, forced: 0, planned: 0 } }
...
export function buildOutagePaths(feed, pal) {
  ...
  const paths = [...byPair.values()].map((p) => {
    ...
    return {
      ...
      width: isForced ? OUTAGE_WIDTH.forced : OUTAGE_WIDTH.planned,
      ...
      dash: p.running > 0 ? OUTAGE_DASH_RUNNING : OUTAGE_DASH_UPCOMING,
    }
  })
  return {
    paths,
