...
export default function PowerMap({ onBorderSelect, onZoneSelect, selectedZone, tall = false }) {
  ...
  const layers = useMemo(() => {
    ...
    // Drawn BELOW the arcs on purpose — the arcs are the always-on default and
    // carry the click that opens the border detail; an opt-in overlay must not
    // swallow that hit where the two geometries converge near the endpoints.
    const outageLayers = overlays.outages && outagePaths.length > 0
      ? makeOutageLayers({ paths: outagePaths, pal })
      : []
