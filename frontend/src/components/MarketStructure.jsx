...
function CurveRow({ name, curve }) {
  ...
  const spreadSign = curve.spread >= 0 ? '+' : ''
  ...
  return (
    <div className="border border-border bg-surface rounded">
      <div className="flex items-center justify-between px-4 py-2 border-b border-border">
        ...
        <span className={`font-mono text-[10px] font-bold ${summaryStyle.text}`}>
          {summaryStyle.label}
        </span>
      </div>
